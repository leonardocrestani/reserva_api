...
describe('Unbook schedule', () => {
  ...
  beforeAll(async () => {
    ...
    await request(app).post('/api/user').send({ ...data, email: 'leonard@test.com' })
  ...
  test('Should get error when trying to unbook schedule with different responsible person email', async () => {
    const data = body
    await request(app).post('/api/place').set('Authorization', `Bearer ${token}`).send(placeBody.body)
    const schedule : any = await request(app).post('/api/schedule').set('Authorization', `Bearer ${token}`).send(data)
    await request(app).put(`/api/schedule/book/${schedule.body._id}`).set('Authorization', `Bearer ${token}`).send({ responsible_person_email: 'leonard@test.com' })
    const response : any = await request(app).put(`/api/schedule/unbook/${schedule.body._id}`).set('Authorization', `Bearer ${token}`).send({ responsible_person_email: userBody.body.email })
    expect(response.status).toBe(400)
    expect(response.body.message).toBe('Usuario desmarcando horario incorreto')
  })
